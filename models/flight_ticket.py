...
class FlightTicket(models.Model):
    ...
    def _compute_ticket_price(self):
        ...
        for rec in self:
            ...
            if cargo_profit > 5000:
                subsidy_pct = min(cargo_profit / 500000, 0.10)
            ...
            if market_avg and running_total > market_avg * 1.15:
                ...
                _logger.warning(
                    'Skyledger Pricing: Ticket %s price $%.2f is >15%% above market avg $%.2f on %s',
                    rec.ticket_ref or '(new)', running_total, market_avg, sched.flight_number
                )
            ...
            if rec.quote_currency_id and rec.quote_currency_id != rec.currency_id:
                rec.final_price_quoted = rec.currency_id._convert(
                    rec.final_price, rec.quote_currency_id, rec.env.company, fields.Date.today()
                )
            else:
    ...
    def _create_invoice(self):
        ...
        for rec in self:
            ...
            if not sc or not sc.product_id:
                _logger.warning('Skyledger: No product mapped for %s – invoice skipped.', rec.ticket_ref)
                continue
            fuel_product_id = int(self.env['ir.config_parameter'].sudo().get_param('aviation_erp.fuel_surcharge_product_id', default=0))
            fuel_product = self.env['product.product'].browse(fuel_product_id) if fuel_product_id else None
            invoice_lines = [
                (0, 0, {
                    'product_id': sc.product_id.id,
                    'name': _('Base Fare – %s (%s)', rec.schedule_id.flight_number, dict(CABIN_CLASSES).get(rec.cabin_class, rec.cabin_class)),
                    'quantity': 1,
            ...
            if rec.fuel_surcharge and fuel_product:
                invoice_lines.append((0, 0, {
                    ...
                    'name': _('Fuel Surcharge – %s (Jet A-1 @ $%.4f/gal)', rec.schedule_id.flight_number, rec.jet_a1_price_snapshot),
                    'quantity': 1,
                    'price_unit': rec.fuel_surcharge,
                }))
            elif rec.fuel_surcharge:
                invoice_lines[0][2]['name'] += _(' (incl. Fuel Surcharge $%.2f)', rec.fuel_surcharge)
            ...
            if rec.loyalty_discount_amount:
                invoice_lines.append((0, 0, {
                    'product_id': sc.product_id.id,
                    'name': _('Frequent Flyer %s Discount (%.0f%%)', rec.loyalty_tier.title() if rec.loyalty_tier else '', rec.loyalty_discount_pct),
                    'quantity': 1,
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('ticket_ref') or vals['ticket_ref'] == '/':
                vals['ticket_ref'] = self.env['ir.sequence'].next_by_code('flight.ticket') or '/'
